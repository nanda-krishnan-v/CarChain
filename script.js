            ...
            <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin-top: 15px; border: 1px solid #dee2e6;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        ...
                        <td style="padding: 8px; border-bottom: 1px solid #dee2e6; word-break: break-all; font-family: monospace; font-size: 0.9em;">${receipt.transactionHash}</td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td style="padding: 8px; border-bottom: 1px solid #dee2e6;">${receipt.blockNumber}</td>
                    </tr>
                    <tr style="background-color: #ffffff;">
...
viewCarBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (error.message.includes('execution reverted') || error.message.includes('revert')) {
            ...
            statusDiv.style.backgroundColor = '#fff3cd';
            statusDiv.style.color = '#856404';
            carDetailsDiv.innerHTML = '<p style="text-align: center; color: #666;">Please register a car first using the form above.</p>';
        } else {
            statusDiv.innerText = '❌ Error: Could not retrieve car details.';
            statusDiv.style.backgroundColor = '#f8d7da';
            statusDiv.style.color = '#721c24';
            carDetailsDiv.innerHTML = '<p style="text-align: center; color: #666;">Please try again or check the console for details.</p>';
        }
    }
});
